        ...
        <section className="print-section surface-card mt-8 overflow-hidden">
          ...
          {report.chili.sales.length === 0 ? <EmptySection label="No chili sales recorded this year." /> : (
            <div className="overflow-x-auto"><table className="ledger-table w-full min-w-[620px] text-left"><thead className="bg-[#f8f7f1] text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground"><tr><th className="px-6 py-3">Recipient</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Grade · quantity · price</th><th className="px-4 py-3">Payment</th><th className="px-6 py-3 text-right">Total</th></tr></thead><tbody className="divide-y divide-[#ece9e0]">
              ...
              <tr className="bg-[#f8f7f1] font-semibold"><td className="px-6 py-3" colSpan={4}>Total</td><td className="px-6 py-3 text-right">{formatMoney(report.chili.sales.reduce((sum, sale) => sum + sale.totalCents, 0))}</td></tr>
              {report.chili.sales.some(sale => sale.paidAt === null) ? <tr className="text-sm"><td className="px-6 py-3 text-muted-foreground" colSpan={4}>Still unpaid</td><td className="px-6 py-3 text-right font-semibold text-[#b34d2e]">{formatMoney(report.chili.sales.filter(sale => sale.paidAt === null).reduce((sum, sale) => sum + sale.totalCents, 0))}</td></tr> : null}
            </tbody></table></div>
          )}
        </section>
      </Reveal>
